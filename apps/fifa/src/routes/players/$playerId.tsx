...
import { computePlayerStats, computeH2H } from "@/lib/stats"
...
function PlayerProfilePage() {
  ...
  const data = useLiveQuery(async () => {
    ...
    const stats = computePlayerStats(allPlayers, allMatches)
    const playerStats = stats.find((s) => s.playerId === playerId)
    const h2h = computeH2H(playerId, allPlayers, allMatches)
    ...
    const last10 = allMatches
      ...
      .map((m) => {
        ...
        if (m.player1Id === playerId) {
          r = m.player1Score > m.player2Score ? "W" : m.player1Score < m.player2Score ? "L" : "D"
        } else {
          r = m.player2Score > m.player1Score ? "W" : m.player2Score < m.player1Score ? "L" : "D"
        }
        return r
      })
                        ...
                        <td className="pl-4 pr-2 py-2.5">
                          <div className="flex items-center gap-2">
                            <div
                              ...
                              style={{ backgroundColor: record.opponentColor }}
                            />
        ...
        {playerMatches.length > 0 && (
          <motion.div variants={itemVariants}>
            ...
            <div className="space-y-2">
              {playerMatches.map((m) => {
                const opp = m.player1Id === playerId
                    ...
                    <CardContent className="py-3 px-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <div
                            className="size-6 rounded-full shrink-0"
                            style={{ backgroundColor: opp?.colorHex ?? "#6b7280" }}
                          />
