...
function NewMatchPage() {
  ...
  function validate() {
    ...
    if (player1Id && player2Id && player1Id === player2Id) {
      e.player2 = "Can't select the same player twice"
    }
    if (player1Score < 0) e.score1 = "Score can't be negative"
    if (player2Score < 0) e.score2 = "Score can't be negative"
    return e
  }
      ...
      <div className="min-h-full bg-background px-4 py-4">
        <Card>
          <CardContent className="py-10 text-center space-y-3">
            ...
            <Button onClick={() => navigate({ to: "/players" })} variant="outline" className="mt-2">
              Add Players
            </Button>
          </CardContent>
        </Card>
  ...
  return (
    <div className="min-h-full bg-background">
      ...
      <form onSubmit={handleSubmit} className="px-4 py-4 space-y-5">
          ...
          <CardContent className="py-4 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Select
                  value={player1Id}
                  onValueChange={(v) => { setPlayer1Id(v); setErrors((e) => ({ ...e, player1: "" })) }}
                >
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={player2Id}
                  onValueChange={(v) => { setPlayer2Id(v); setErrors((e) => ({ ...e, player2: "" })) }}
                >
          ...
          <CardContent className="py-6">
            <div className="flex items-center justify-around">
              <ScoreInput
                ...
                playerName={p1?.name}
                playerColor={p1?.colorHex}
              />

              <div className="flex flex-col items-center gap-1">
                <span className="text-2xl text-muted-foreground font-light">–</span>
                {player1Score === player2Score && player1Score >= 0 && player1Id && player2Id && (
                  <span className="text-[10px] font-semibold text-amber-500">DRAW</span>
                )}
                {player1Score !== player2Score && player1Id && player2Id && (
                  <span className="text-[10px] font-semibold text-emerald-500">
                    {player1Score > player2Score ? p1?.name?.split(" ")[0] : p2?.name?.split(" ")[0]} wins
                  </span>
                )}
              </div>
        ...
        {/* Submit */}
        <motion.div whileTap={{ scale: 0.98 }} className="pb-4">
          <Button
            type="submit"
